...
- 侧边菜单（点击 app icon 打开）：导出 JSON、导入 JSON、检查更新、主题切换、语言切换、关于
- 刷新页面数据不丢失
...
- 菜单项：导出、导入（隐藏 file input）、**检查更新**、主题（→ 右侧子面板，三选一）、语言（→ 右侧子面板）、关于（→ 居中弹窗）
- 主题与语言子面板互斥，点击抽屉空白处同时关闭
- "检查更新"：调用 `window.todoCheckForUpdates()`，检查中禁用按钮；结果用共享 alert 弹窗展示（成功 / 失败 / 离线）
- 关于弹窗展示 `techs`、`version`、`repository`（可点击链接）以及配置缓存时间（`appConfigCachedAt`，未缓存时显示"尚未缓存"）
...
`t` 包含：`placeholder, add, filters, empty, confirmDelete, exportData, importData, importError, language, about, close, aboutDesc, markDone, markUndone`，4 条优先级标签文本，关于弹窗字段 `techs, version, repository, techsLabel, versionLabel, repositoryLabel, cachedLabel, configCachedTime, configCachePending`，主题标签 `theme, themeSystem, themeLight, themeDark`，以及更新流程文案 `checkUpdates, checkingUpdates, updateSuccess, updateFailed, updateOffline`。
